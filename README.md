# 👋 &nbsp;Hi there
Software Engineer at Presmatics 🚀

Building production-grade applications with PHP, Laravel, and ASP.NET Core.

**Currently:** Full Stack Developer at Presmatics | Leading Wasalni (Personal Project)  
**Wasalni:** Smart Delivery Platform - Graduation Project (Launching Soon)  
**Previous:** Complex E-Commerce Platforms | Real-time Chat Systems (WhatsApp-like) | Payment Integrations

**Tech Stack:** PHP | Laravel | ASP.NET Core | React | MySQL | Stripe  
**Certification:** Cisco CCNA

Passionate about scalable architecture, clean code, and solving complex problems.  
Open to collaboration and mentoring.

&nbsp;

## 💻 &nbsp;My Toolbox

<img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/php/php-original.svg" alt="PHP" width="50" height="50"/> &nbsp;<img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/csharp/csharp-original.svg" alt="C#" width="50" height="50"/> &nbsp;<img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/dot-net/dot-net-original.svg" alt="ASP.NET" width="50" height="50"/> &nbsp;<img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/react/react-original.svg" alt="React" width="50" height="50"/> &nbsp;<img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/javascript/javascript-original.svg" alt="JavaScript" width="50" height="50"/> &nbsp;<img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/mysql/mysql-original.svg" alt="MySQL" width="50" height="50"/> &nbsp;<img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/git/git-original.svg" alt="Git" width="50" height="50"/> &nbsp;<img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/vscode/vscode-original.svg" alt="VSCode" width="50" height="50"/>

&nbsp;

## 🚀 &nbsp;Personal Projects

- **Wasalni** - Smart Delivery Platform (Graduation Project)
  - Real-time tracking, Multi-vendor management, Complex logistics optimization
  - Tech: PHP/Laravel, React, MySQL, WebSockets

- **E-Commerce Platform** - Multi-vendor Marketplace
  - Admin dashboard, Vendor management, Stripe payments, Quantity-based discounts, Coupons
  - Tech: PHP/Laravel & ASP.NET Core versions

- **Real-time Chat Application** - WhatsApp-like messaging
  - WebSocket integration, Message history, User presence
  - Tech: PHP/ASP.NET Backend, React Frontend

&nbsp;

## 📈 &nbsp;About Me

Full Stack Developer with hands-on experience building complex, scalable applications. Specialized in backend architecture and payment systems integration.

**Experience:**
- Software Engineer at Presmatics
- Software Development Intern at Flothers
- Modern Academy Maadi Graduate

&nbsp;

## 🎯 &nbsp;What I'm Focused On

- Building production-ready systems
- Database optimization & architecture
- Payment integrations (Stripe)
- Real-time applications
- Mentoring junior developers

&nbsp;

## 📞 &nbsp;Get In Touch

- **LinkedIn:** [Mazen Mohamed](https://www.linkedin.com/in/mazen-mohamed-2b8518405/?lipi=urn%3Ali%3Apage%3Ad_flagship3_profile_view_base_contact_details%3B07V4jYdHSjuQJNV9fOV%2FMg%3D%3D)
- **Email:** mazentohameee@gmail.com
- **GitHub:** [Mazen](https://github.com/mazen42)

---

*Always interested in discussing scalable architecture, payment systems, and logistics solutions.*
